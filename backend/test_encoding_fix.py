...
async def test_sse_encoding():
    ...
    for i, message in enumerate(test_messages):
        ...
        try:
            # This is similar to what happens in the generate() function
            status_data = json.dumps({'status': 'connected'}, ensure_ascii=False)
            print(f"  Status message (UTF-8): data: {status_data}")
            
            content_data = json.dumps({'content': message}, ensure_ascii=False)
            print(f"  Content message (UTF-8): data: {content_data}")

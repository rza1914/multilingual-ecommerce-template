      ...
      <form onSubmit={handleSubmit} className="relative">
        <div className="flex">
          <input
            ...
            placeholder={t('search.placeholder', 'جستجوی هوشمند محصولات... (مثلاً \'گوشی سامسونگ زیر 20 میلیون\')')}
          ...
          <button
            type="submit"
            disabled={loading}
            className="bg-gradient-to-r from-orange-500 to-orange-600 text-white px-6 rounded-r-lg hover:from-orange-600 hover:to-orange-700 focus:outline-none focus:ring-2 focus:ring-orange-500 disabled:opacity-50"
          >
            {loading ? (
              ...
              <div className="p-4">
                ...
                {Object.keys(results.extracted_filters).length > 0 && (
                  <div className="mb-3 text-xs text-gray-500 dark:text-gray-400 flex flex-wrap gap-1">
                    {results.extracted_filters.max_price && (
                      <span className="bg-orange-100 text-orange-800 px-2 py-1 rounded">تا {formatPrice(results.extracted_filters.max_price)} تومان</span>
                    )}
                ...
                <div className="space-y-3">
                  {results.results.length === 0 ? (
                    <div className="text-center py-4 text-gray-500">{t('search.no_results', 'محصولی یافت نشد')}</div>
                  ) : (

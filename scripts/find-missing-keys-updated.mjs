...
// The Arabic file has a different structure - all content is under "translation" key
...
const arFooterKeys = ar.footer ? getAllKeys(ar.footer, 'footer') : [];
...
const missingInArFooter = footerComponentKeys.filter(k => !arFooterKeys.includes(k));
...
if (missingInArFooter.length > 0) {
  console.log(`\n   ❌ Keys used by component but MISSING in ar.json (${missingInArFooter.length}):`);
  missingInArFooter.forEach(k => console.log(`      - ${k}`));
}
